...
import { InstagramAdapter } from "../src/platforms/instagram.js";
import { existsSync, unlinkSync, readFileSync } from "node:fs";

// Real end-to-end Instagram Business test, single phase — same
// signal-file-polling pattern as the other sandbox-e2e scripts. Uses the
// SAME Meta app/credentials as Facebook (Facebook Login for Business), just
// a different redirect signal file so the two tests don't collide if run
// close together.
const TEST_IMAGE_URL = "https://lazyrelay.com/favicon.png";
const CODE_FILE = "instagram-oauth-code.txt";
...
async function main() {
  ...
  const email = `instagram-sandbox-test-${Date.now()}@lazyrelay.invalid`;
  ...
  const adapter = new InstagramAdapter(clientId, clientSecret, redirectUri);
  const state = `test-${Date.now()}`;
  const authorizeUrl = await adapter.getAuthorizeUrl(state);
  console.log("\nOpen this URL in a browser, log in as the account that manages the target Page/Instagram Business Account, approve, then");
  ...
  console.log(verifyResult.verifiedLive ? "ALL PASS — post is verified live on Instagram" : "PARTIAL — post() succeeded but verifyPublished() did not confirm live");
