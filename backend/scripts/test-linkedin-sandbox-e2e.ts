...
import { LinkedInAdapter } from "../src/platforms/linkedin.js";
import { existsSync, unlinkSync, readFileSync } from "node:fs";

// Real end-to-end LinkedIn test, single phase — polls a signal file for
// the OAuth code (same pattern as test-mastodon-sandbox-e2e.ts) so one
...
const CODE_FILE = "linkedin-oauth-code.txt";
...
async function main() {
  ...
  const redirectUri = process.env.LINKEDIN_REDIRECT_URI || "http://localhost:9999/callback";
  ...
  const email = `linkedin-sandbox-test-${Date.now()}@lazyrelay.invalid`;
  ...
  const adapter = new LinkedInAdapter(clientId, clientSecret, redirectUri);
  const state = `test-${Date.now()}`;
  const authorizeUrl = await adapter.getAuthorizeUrl(state);
  console.log("\nOpen this URL in a browser, log in as the LinkedIn account to connect, approve, then");
  ...
  const postResult = await adapter.post({
    ...
    content: "LazyRelay LinkedIn adapter end-to-end test post",
  ...
  console.log(verifyResult.verifiedLive ? "ALL PASS — post is verified live on LinkedIn" : "PARTIAL — post() succeeded but verifyPublished() did not confirm live");
