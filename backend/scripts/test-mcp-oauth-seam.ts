...
//
// Usage:
//   Step A:  npx tsx scripts/test-mcp-oauth-seam.ts
//              -> checks the toggle, registers a client, prints a URL to open
//   Step B:  npx tsx scripts/test-mcp-oauth-seam.ts <code> <client_id> <verifier>
...
if (!code) {
  ...
  console.log(`\nThen run:\n  npx tsx scripts/test-mcp-oauth-seam.ts <code> ${reg.client_id} ${verifier}`);
  process.exit(0);
}
...
const { SupabaseMcpTokenVerifier } = await import("../src/http/mcpAuth.js");
try {
