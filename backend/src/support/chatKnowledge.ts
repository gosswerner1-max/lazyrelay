...
- TikTok: no way to choose a custom cover/thumbnail frame, and no trending-sound selection -- LazyRelay posts the video as uploaded. TikTok's own API allows up to 4GB, but LazyRelay's own media-upload endpoint caps every file at 45MB regardless of platform -- that's the real ceiling in practice.
- TikTok posting is temporarily not working (as of 2026-09-04) while we resolve a technical requirement on TikTok's side -- if a customer asks why their TikTok post hasn't gone out, or whether TikTok is working, say plainly that we're aware and are busy fixing a technicality with TikTok, no ETA to share yet. Don't speculate about the cause or timeline beyond that. Every other platform is unaffected.
- YouTube: real capacity is Shorts-length video, not full-length uploads -- LazyRelay's media-upload cap is 45MB, which comfortably covers a normal Short but not a longer YouTube video. A customer wanting to publish a longer video should upload it directly on YouTube. LazyRelay always does a standard upload -- whether YouTube then classifies it as a Short is entirely YouTube's own decision based on the video itself (vertical + under 3 min), not a choice LazyRelay makes. Video title comes from the first ~100 characters of the post content and description from the first ~5000 -- there's no separate title field, and tags aren't supported. Videos always publish as Public -- no Unlisted/Private option.
...
- High-resolution video failing to upload while smaller files work: most platforms don't have a resolution-specific check yet, so a large file usually hits LazyRelay's generic 20MB fallback size cap rather than anything platform-specific -- try compressing it.
...
- A dedicated "Social Inbox" isn't one single named feature, but its core job is already covered by the Mentions and DMs tabs (reading/replying across platforms, plus AI-assisted triage and the private-reply automation above) -- reframe a request for that as already substantially covered, not a gap, unless the customer means something more specific.
