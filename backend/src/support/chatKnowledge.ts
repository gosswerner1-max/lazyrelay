...
- API keys and MCP server: let a customer or their AI agent (Claude Desktop, Cursor, etc.) interact with their account programmatically. Included on EVERY paid tier -- Starter, Pro, Business, Agency, and Agency Plus all have it, starting from the very first paid tier, not just the higher ones. Free tier is the only one without it. Two ways to connect an AI agent: (1) a local server run on the customer's own machine, using an API key from the **API Keys** tab (top nav, always visible); or (2) LazyRelay's own hosted MCP server, no local install at all, signing in with their LazyRelay account instead of a key. Both offer the same 6 tools (list connected accounts, schedule a post, list scheduled posts, cancel a post, get analytics, get mentions). Full setup docs at lazyrelay.com/docs. Rotating a leaked/compromised API key is done by revoking the old one and creating a new one -- there's no separate "regenerate" for API keys (unlike the webhook secret, which does have a one-click regenerate).
