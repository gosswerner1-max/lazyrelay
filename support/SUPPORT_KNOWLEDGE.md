...
| TikTok | **Not available for public TikTok accounts.** The app is approved (2026-08-14) but the client is unaudited for the Content Posting API, so a public account gets `403 unaudited_client_can_only_post_to_private_accounts` whatever privacy level is requested | — | Direct Post audit rejected 2026-09-14 (ref `20260905111305`), **resubmitted 2026-09-15, in review** (ref `20260915123859`, TikTok's estimate 2-4 weeks). **Customer wording: TikTok posting isn't available yet and is waiting on TikTok's approval.** Give no date, never suggest they change account privacy, never blame their settings |
...
| Customer says... | Likely cause | Fix |
|---|---|---|
| "My video only posts privately, followers can't find it" | TikTok forces ALL unaudited-client posts to self-only visibility, regardless of what privacy setting the user picked | **Audit status is now KNOWN, 2026-09-05: LazyRelay's client is unaudited and an audit application is in review (submitted 2026-09-05, TikTok estimate 2-4 weeks).** **Update 2026-09-14: that application was REJECTED, so replace "under review" below with "waiting on TikTok's approval", still no date.** So this is expected, not a bug, and not fixable per-customer. In practice it presents as a hard failure rather than a private post — a public TikTok account returns `403 unaudited_client_can_only_post_to_private_accounts`. Tell the customer plainly that TikTok posting is awaiting TikTok's own approval, give no date beyond "under review", and never blame their account settings |
...
|---|---|---|---|---|
| **TikTok Direct Post audit** (`noreply@dev.tiktok.com` sends automated notices; `tiktokfordevelopers@tiktok.com` answers support ticket `8ed39c9bce67f365`) | `accounts@` | **In review.** Resubmitted 2026-09-15, ref `20260915123859` | A verdict mail quoting `20260915123859` | **A verdict is a status change: post it to Slack** and update Current product state plus Part 1's TikTok self-only row in the same pass. "A decision is delayed" is not a status change. Match on subject and content, never on sender (three sender shapes). The rejection reason sits in a hover tooltip on the ⓘ icon next to "Reapply" in the portal and is not customer-facing. No reply to `noreply@` |
...
### Snapchat chase-pattern watch (live — the 10:00 UTC run on 2026-09-16 must read this)
Agent sends to `profile-api-dev-support@snapchat.com` went out **2026-08-19, 08-26 and 09-02, each at the 08:22 UTC slot**, each with a doubled signature. **2026-09-09: no send** (checked 09:00 UTC, after the slot — a genuine miss). **2026-09-16: not yet scored.** The 06:00 UTC run was 2h22m before the slot, so its "no send yet" reading means nothing. The task's cron runs at 01/04/07/10/13/16/19/22 UTC, so **the 10:00 UTC run is the first that can score it**. Check `accounts@` `INBOX.Sent` for anything newer than uid 31 (2026-09-02 08:22:22 UTC).
...
- **Known loose ends (checked 2026-09-16):** LaunchIgniter (2026-09-04) and LaunchBuck (2026-09-06) earned candidate verdicts but are **not in the tracker CSV**. Crozdesk's row reads `Submitted` (2026-08-28). Its Revleads acceptance gave 30 days (~2026-09-27) to finish the listing, and whether that happened isn't recorded.
